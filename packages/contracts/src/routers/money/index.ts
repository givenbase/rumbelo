...
export const contract = {
  jars: {
    ...
    update: oc.input(S.Jar.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.Jar),
    updateSplit: oc.input(S.UpdateJarSplit).output(z.array(S.Jar)),
    createCategory: oc
      .input(z.object({ householdId: S.HouseholdId, jarId: S.Id, name: z.string().min(1).max(80), budgeted: S.Money }))
      .output(S.Category),
    updateCategory: oc.input(S.Category.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.Category),
  ...
  income: {
    ...
    update: oc.input(S.IncomeSource.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.IncomeSource),
    ...
    applySplit: oc
      .input(z.object({ householdId: S.HouseholdId, incomeSourceId: S.Id, amount: S.Money, bookedOn: S.IsoDate }))
      .output(z.object({ allocations: z.array(z.object({ jarId: S.Id, amount: S.Money })) })),
  },

  fixedCosts: {
    list: oc.input(S.HouseholdScoped.extend({ direction: z.enum(['IN', 'OUT']).nullish() })).output(z.array(S.FixedCost)),
    ...
    update: oc.input(S.FixedCost.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.FixedCost),
    remove: oc.input(z.object({ householdId: S.HouseholdId, id: S.Id })).output(ok),
  },

  accounts: {
    list: oc.input(S.HouseholdScoped).output(z.array(S.Account)),
    create: oc.input(S.Account.omit({ id: true, connectionId: true, lastSyncedAt: true })).output(S.Account),
  },
  ...
  transactions: {
    ...
    update: oc.input(S.Transaction.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.Transaction),
    sort: oc.input(S.SortTransaction).output(S.Transaction),
    bulkSort: oc
      .input(z.object({ householdId: S.HouseholdId, transactionIds: z.array(S.Id).min(1), jarId: S.Id, categoryId: S.Id.nullish() }))
  ...
  rules: {
    ...
    update: oc.input(S.Rule.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.Rule),
    remove: oc.input(z.object({ householdId: S.HouseholdId, id: S.Id })).output(ok),
  ...
  goals: {
    ...
    update: oc.input(S.Goal.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.Goal),
    remove: oc.input(z.object({ householdId: S.HouseholdId, id: S.Id })).output(ok),
  ...
  debts: {
    ...
    update: oc.input(S.Debt.partial().extend({ id: S.Id, householdId: S.HouseholdId })).output(S.Debt),
    remove: oc.input(z.object({ householdId: S.HouseholdId, id: S.Id })).output(ok),
    plan: oc.input(S.HouseholdScoped.extend({ strategy: S.PayoffStrategy.default('AVALANCHE') })).output(S.DebtPlan),
  },

  turn: {
    current: oc.input(S.HouseholdScoped.extend({ period: S.PeriodKey.nullish() })).output(S.Turn),
  ...
  ritual: {
    current: oc.input(S.HouseholdScoped.extend({ week: S.WeekKey.nullish() })).output(S.WeeklyRitual),
    advance: oc
      .input(z.object({
  ...
  dashboard: {
    get: oc.input(S.HouseholdScoped.extend({ period: S.PeriodKey.nullish() })).output(S.Dashboard),
  },
};
