...
export function JarCard({ jar }: { jar: JarCardModel }) {
  ...
  const usedPct =
    jar.allocated > 0
      ? Math.min(100, Math.round((spent / jar.allocated) * 100))
      : 0;
  ...
  return (
    ...
    >
      ...
      {guide && (
        <p className="mt-3.5 text-sm leading-relaxed text-pretty text-fg-muted">{guide.note}</p>
      )}

      <p className="mt-3 font-mono text-xs font-medium leading-relaxed text-fg-faint">
        {activity}
      </p>
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="mt-3.5 w-full rounded-full border border-accent/40 bg-accent-soft px-2.5 py-2.5 font-mono text-xs font-medium tracking-wide text-accent uppercase transition-colors hover:bg-accent-soft"
      >
                  ...
                  <div key={sub.label}>
                    <div className="mb-1 flex justify-between gap-2.5 font-mono text-xs font-medium">
                      ...
                      <span className="text-accent">
                        {formatMoney(Math.round((jar.allocated * sub.pct) / 100))} · {sub.pct}%
                      </span>
                    </div>
      ...
      <div className="mt-3 flex gap-2">
        <button
          ...
          className="flex-1 rounded-full border border-line-strong bg-transparent px-2 py-2 font-mono text-xs font-medium tracking-wide text-fg-secondary uppercase transition-colors hover:border-accent hover:text-accent"
        >
          Add a spend
        </button>
        <button
