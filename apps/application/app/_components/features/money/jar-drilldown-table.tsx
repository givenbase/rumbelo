 ...
 *
 * Renders every jar as a single-open accordion row. Clicking a row expands a
 * 4-column category table (Categorie / Gepland / Besteed / Mee-/overschot).
 * Only one jar is open at a time so the overview stays uncluttered.
 */
export function JarDrilldownTable({ jars }: { jars: Jar[] }) {
  const [openId, setOpenId] = useState<string | null>(null);

  return (
    <div className="grid">
      {jars.map((jar) => {
        const open = openId === jar.id;
        const usedPct =
          jar.allocated > 0
        ...
        return (
          <div key={jar.id} className="border-b border-line last:border-b-0">
            <button
              type="button"
              onClick={() => setOpenId(open ? null : jar.id)}
            ...
            >
              ...
              <span className="flex min-w-0 items-center gap-2.5">
                <span className="grid size-7.5 shrink-0 place-items-center rounded-[9px] border border-line bg-raised text-sm">
                  {jar.icon}
                </span>
                <span className="grid min-w-0 gap-0.5">
                  <span className="truncate text-sm text-fg">{jar.name}</span>
                  <span className="truncate font-mono text-[10px] tracking-[0.08em] text-fg-faint">
                    {jar.subtitle}
                  </span>
                </span>
              </span>
              ...
              <span className="text-right">
                <div className={cn('font-mono text-[13px]', jar.overspent ? 'text-danger' : 'text-fg')}>
                  {formatMoney(jar.remaining)}
                </div>
                <div className="font-mono text-[10px] text-fg-faint">
                  van {formatMoney(jar.allocated)}
                </div>
              </span>
              ...
              <span
                className={cn(
                  'text-center text-xs text-fg-faint transition-transform duration-200',
                  open && 'rotate-180',
                )}
              >
                ▾
            ...
            {open && (
              <div className="mb-3 ml-11.5 grid animate-rise">
                ...
                <div className="grid grid-cols-[minmax(0,1fr)_80px_80px_90px] gap-3 pb-2 text-[9px] font-medium tracking-[0.14em] text-fg-faint uppercase">
                  ...
                  <span className="text-right">Besteed</span>
                  <span className="text-right">Mee-/overschot</span>
                </div>
                {jar.categories.length === 0 ? (
                  <p className="border-t border-line py-1.5 text-[12.5px] text-fg-faint">
                    Geen categorieën.
                  </p>
                ) : (
                  jar.categories.map((c) => {
                    ...
                    return (
                      <div
                        key={c.id}
                        className="grid grid-cols-[minmax(0,1fr)_80px_80px_90px] items-baseline gap-3 border-t border-line py-1.5"
                      >
