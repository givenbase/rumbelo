...
export class HouseholdService {
  ...
  async members(householdId: string) {
    const rows = await this.em.getConnection().execute<
      { id: string; userId: string; role: string; name: string; email: string; image: string | null }[]
    >(
  ...
  private async onboardInternal(input: z.infer<typeof OnboardingInput>, headers: Headers) {
    const userId = currentUserId();
    const splitTotal = input.split.reduce((sum: number, s: { percentage: number }) => sum + s.percentage, 0);
    ...
    const splitByKey = new Map(
      input.split.map((s: { key: string; percentage: number }) => [s.key.toUpperCase(), s.percentage]),
    );
