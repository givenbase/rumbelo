...
export class LogService {
  ...
  async logs() {
    ...
    return rows.map((l) => ({
      id: l.id, householdId: l.householdId, userId: l.userId,
      on: l.loggedOn, metric: l.metric, value: Number(l.value), note: l.note,
    }));
  }
  ...
  async create(input: {
    ...
    let row = await this.repo.findOne({ userId, loggedOn: input.on, metric: input.metric as EnergyMetric });
    if (row) {
  ...
  async summary() {
    ...
    return Object.values(EnergyMetric).map((metric) => {
      ...
      const avg = (n: number) => {
        ...
        return slice.length ? slice.reduce((s, r) => s + Number(r.value), 0) / slice.length : 0;
      };
      const a7 = avg(7);
      const a28 = avg(28);
      return {
        ...
        trend: a7 > a28 + 2 ? ('UP' as const) : a7 < a28 - 2 ? ('DOWN' as const) : ('FLAT' as const),
