...
export class DebtService {
  ...
  async plan(strategy: PayoffStrategy) {
    ...
    if (debts.length === 0) {
      return {
        strategy, totalBalance: 0, totalInterestProjected: 0,
        debtFreeOn: null, monthsRemaining: null, order: [],
      };
    }
    ...
    const monthlyPool = debts.reduce((s, d) => s + Number(d.minimumPayment) + Number(d.extraPayment), 0);
...
export function toDto(d: Debt) {
  return {
    id: d.id, householdId: d.householdId, name: d.name, kind: d.kind,
    balance: Number(d.balance), originalBalance: Number(d.originalBalance),
    interestRate: Number(d.interestRate), minimumPayment: Number(d.minimumPayment),
    extraPayment: Number(d.extraPayment), dueDay: d.dueDay, closedOn: d.closedOn,
  };
}
