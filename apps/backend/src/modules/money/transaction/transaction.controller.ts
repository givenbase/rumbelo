...
export class TransactionController {
  ...
  inbox() {
    return implement(contract.money.transactions.inbox).handler(() => this.transactions.inbox());
  }
  ...
  create() {
    return implement(contract.money.transactions.create).handler(({ input }) => this.transactions.create(input));
  }
  ...
  update() {
    return implement(contract.money.transactions.update).handler(({ input }) =>
      this.transactions.update(input.id, {
        description: input.description, amount: input.amount, note: input.note,
