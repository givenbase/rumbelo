...
export class TransactionService {
  ...
  async create(input: {
    accountId?: string | null; jarId?: string | null; categoryId?: string | null;
    amount: number; bookedOn: string; description: string;
    ...
    const entity = this.em.create(Transaction, {
      ...
      dedupeKey: dedupeKey(input.accountId ?? null, input.bookedOn, input.amount, input.description),
    } as never);
  ...
  async update(id: string, patch: Partial<Pick<Transaction, 'description' | 'amount' | 'note' | 'status'>>) {
  ...
  async importCsv(accountId: string, content: string, dryRun: boolean) {
    ...
    const existing = keys.length ? await this.transactions.find({ dedupeKey: { $in: keys } }) : [];
    ...
    if (!dryRun) {
      parsed.forEach((row, i) => {
        ...
        this.em.create(Transaction, {
          ...
          amount: row.amount, bookedOn: row.bookedOn, description: row.description,
          counterparty: row.counterparty, status: TransactionStatus.INBOX,
          source: TransactionSource.CSV, dedupeKey: keys[i]!,
        } as never);
      });
      await this.em.flush();
    }
...
export function toDto(t: Transaction) {
  return {
    id: t.id, householdId: t.householdId,
    accountId: t.account?.id ?? null, jarId: t.jar?.id ?? null, categoryId: t.category?.id ?? null,
    amount: Number(t.amount), bookedOn: t.bookedOn, description: t.description,
    counterparty: t.counterparty, status: t.status, source: t.source,
    appliedRuleId: t.appliedRuleId, note: t.note, createdAt: t.createdAt.toISOString(),
  };
}

/** Stable across re-imports so the same statement never duplicates rows. */
function dedupeKey(accountId: string | null, bookedOn: string, amount: number, description: string) {
  return createHash('sha256')
    .update([accountId ?? '', bookedOn, String(amount), description.trim().toLowerCase()].join('|'))
    .digest('hex')
    .slice(0, 64);
}
