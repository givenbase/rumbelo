...
export class AccountService {
  ...
  async create(input: { name: string; iban?: string | null; kind: string; balance: number }) {
    const account = this.em.create(BankAccount, {
      ...
      name: input.name, iban: input.iban ?? null,
      kind: input.kind as AccountKind, balance: input.balance,
    } as never);
...
export function toDto(a: BankAccount) {
  return {
    id: a.id, householdId: a.householdId, name: a.name, iban: a.iban, kind: a.kind,
    balance: Number(a.balance), connectionId: a.connectionId,
