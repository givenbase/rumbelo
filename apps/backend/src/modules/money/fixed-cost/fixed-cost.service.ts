...
export class FixedCostService {
  ...
  async byJar() {
    ...
    const groups = new Map<string, { jarKey: string; jarName: string; items: ReturnType<typeof toDto>[] }>();
...
export function toDto(f: FixedCost) {
  return {
    id: f.id, householdId: f.householdId, jarId: f.jar.id, categoryId: f.category?.id ?? null,
    name: f.name, amount: Number(f.amount), cadence: f.cadence, dueDay: f.dueDay,
    direction: f.direction, active: f.active, endsOn: f.endsOn, note: f.note,
  };
}
